...
def generate_prompt(args):
    ...
    for per_data in data[start:end]:
        if args.dataset == 'HumanEval':
            ...
            message =[{'role': 'system', 'content': MBPP_system_message}, {'role': 'user', 'content': MBPP_Zeroshot_prompt.format(prompt=prompt, function_name=function_name)}]
        elif args.dataset == 'DS1000':
            prompt = per_data['prompt']
...
def Zeroshot_generate_result(args):

...
def generate_prompt(args):
    ...
    for per_data in data[start:end]:
        if args.dataset == 'HumanEval':
            ...
            message =[{'role': 'system', 'content': HumanEval_system_message}, {'role': 'user', 'content': HumanEval_Persona_prompt.format(prompt=prompt)}]
        elif args.dataset == 'MBPP':
            function_name = get_function_info(per_data['code'])
            prompt = per_data['text']
            message =[{'role': 'system', 'content': MBPP_system_message}, {'role': 'user', 'content': MBPP_Persona_prompt.format(prompt=prompt, function_name=function_name)}]
        elif args.dataset == 'DS1000':
            prompt = per_data['prompt']
            message =[{'role': 'system', 'content': DS1000_system_message}, {'role': 'user', 'content': DS1000_Zeroshot_prompt.format(prompt=prompt)}]
        messages.append(message)

    return messages, data_selected

def Persona_generate_result(args):
    ...
    for idx, per_data in enumerate(tqdm.tqdm(data)):
        ...
        # input_token, input_cost = num_tokens_from_messages(messages[idx], True)
        # output_token, output_cost = num_tokens_from_messages(response, False)
